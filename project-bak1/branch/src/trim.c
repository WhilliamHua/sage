   ...
   if(security_mode==show_master)
   {
	    ...
	    while(sram_addr.word< jj.word)    // affirm that 64*sata.fis_seccnt is right
	    {
		 ...
		 SFR_ext_sram_cntl = 0x10;  //enable addr auto add one
	        if((SFR_ext_sram_data == 0x00)&&(SFR_ext_sram_data == 0x00)) {break; } //check the Range Length, zero:break
	        //LBA偏移操作
...
void trim_burst(u32_t idata block_num)
{
    ...
    enable_fetch_lba;
    myprintf("\nTRIM: %x %x %x %x %x %x ",sata.SUP_LBA5,sata.SUP_LBA4,sata.SUP_LBA3,sata.SUP_LBA2,sata.SUP_LBA1,sata.SUP_LBA0);
    disable_fetch_lba;
