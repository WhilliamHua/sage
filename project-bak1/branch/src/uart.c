...
#ifdef UART_security
	else if(uart_cmd_byte == UART_CONTROL_CMD)
	{
        if(uart_b3==0x53)//S
         { if(uart_b2==0x4F)//O
            if(uart_b1==0x50)//P
            if(uart_b0==0x31)//partiont 1
            	{
              security_mode=show_user;
		uart_show=1;
            	}
	     else if(uart_b0==0x32)
		 {
              security_mode=show_master;
		uart_show=1;
            	}
         }
	 else if(uart_b3==0x45)//E
         { if(uart_b2==0x52)//R
            if(uart_b1==0x41)//A
            if(uart_b0==0x31)//partiont 1
              security_erase=show_user;//erase user partition
	     else if(uart_b0==0x32)
		 security_erase=show_master;//erase master partiton
         }	 
	}
#endif
